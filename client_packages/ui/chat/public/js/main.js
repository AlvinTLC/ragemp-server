...
function enableChatInput(enable) {
    ...
    if (enable != (chat.input != null)) {
        ...
        if (enable) {
            chat.input = $("#chat")
                .append('<div><input id="chat_msg" type="text" /></div>')
                .children(":last");
...
$(document).ready(function () {
    ...
    $("body").keyup(function (event) {
        if (event.which == 84 && chat.input == null && chat.active == true) {
            ...
            if (chat.historyMsgs.length === 0) return;

            const previousMessages = chat.historyMsgs;

            if (previousMessages.length === chat.currentIndex) return;

            chat.input
                .children("input")
                .val(previousMessages[chat.currentIndex]);

            setTimeout(() => {
                chat.input
                    .children("input")
                    .setSelectionRange(
                        previousMessages[chat.currentIndex].length,
                        previousMessages[chat.currentIndex].length
                    );
            }, 1);
